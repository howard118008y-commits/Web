...
def calc_terms(city, num):
    return [
        ...
        (f"累進起點地價（{city}）", f"{city} 115-116 年累進起點地價為 {num} 元；課稅地價總額未超過此數按 10‰ 課徵，超過部分依超過倍數按 15‰～55‰ 累進。"),
...
def guide_terms(city, num):
    return [
        (f"累進起點地價（{city}）", f"計算地價稅級距的基準，每兩年由各縣市重新公告；{city} 115-116 年為 {num} 元，課稅地價與此數的倍數決定適用稅率級距。"),
